...
from sqlalchemy.orm import Mapped, backref, deferred, relationship, synonym, column_property
from sqlalchemy.orm import SynonymProperty, ColumnProperty
...
Base: Any = declarative_base(cls=_Base)
...
class Author(Base):
    ...
    aliases: Mapped[List["Alias"]] = relationship(
        'Alias', back_populates='author', lazy='joined')
    webpages: Mapped[List["AuthorUrl"]] = relationship(
        'AuthorUrl', back_populates='author')
    books: Any = association_proxy('books', 'book')
...
class Book(Base):
    ...
    subjects: Mapped[List["Subject"]] = relationship(
        'Subject', secondary='mn_books_subjects')
    categories: Mapped[List["Category"]] = relationship(
        'Category', secondary='mn_books_categories')
    bookshelves: Mapped[List["Bookshelf"]] = relationship(
        'Bookshelf', secondary='mn_books_bookshelves')
    loccs: Mapped[List["Locc"]] = relationship(
        'Locc', secondary='mn_books_loccs')
    langs: Mapped[List["Lang"]] = relationship(
        'Lang', secondary='mn_books_langs')
    attributes: Mapped[List["Attribute"]] = relationship(
        'Attribute',
        uselist=True,
        back_populates='book',
        order_by='Attribute.fk_attriblist',
    ...
    authors: Mapped[List["BookAuthor"]] = relationship(
        ...
        uselist=True,
        back_populates='book',
        order_by='BookAuthor.heading, BookAuthor.role, BookAuthor.name',
        cascade="all, delete-orphan"
    )
...
class Category(Base):
    ...
    books: Mapped[List["Book"]] = relationship(
        'Book', secondary='mn_books_categories', back_populates="categories")
...
class Permission(Base):
    ...
    users: Mapped[List["User"]] = relationship(
        'User', secondary='mn_users_permissions')
...
class Attribute(Base):
    ...
    attribute_type: Mapped[Optional["Attriblist"]] = relationship('Attriblist')
    book: Mapped["Book"] = relationship(
        'Book', back_populates='attributes')
    lang: Mapped[Optional["Lang"]] = relationship('Lang')
...
class AuthorUrl(Base):
    ...
    author: Mapped[Optional["Author"]] = relationship(
        'Author', back_populates='webpages')
...
class File(Base):
    ...
    book: Mapped[Optional["Book"]] = relationship(
        'Book', back_populates="files")
    compression_type: Mapped[Optional["Compression"]] = relationship(
        'Compression')
    encoding_type: Mapped[Optional["Encoding"]] = relationship(
        'Encoding')
    file_type: Mapped[Optional["Filetype"]] = relationship(
        'Filetype', lazy='joined')

    filetype: SynonymProperty[Optional[str]] = synonym('fk_filetypes')
    encoding: SynonymProperty[Optional[str]] = synonym('fk_encodings')

    ftsortorder: ColumnProperty = column_property(
        select(Filetype.sortorder).where(Filetype.pk == fk_filetypes).scalar_subquery()
    )
    encsortorder: ColumnProperty = column_property(
        select(Encoding.sortorder).where(Encoding.pk == fk_encodings).scalar_subquery()
    )

    generated: Any = association_proxy('file_type', 'generated')
    mediatype: Any = association_proxy('file_type', 'mediatype')
...
class BookAuthor(Base):
    ...
    role_type: Mapped[Optional["Role"]] = relationship(
        'Role', backref=backref("authors", cascade="all"), uselist=False)
    book: Mapped["Book"] = relationship(
        "Book", back_populates="authors")
    author: Mapped[Optional["Author"]] = relationship(
        "Author",
        backref=backref("authorbooks", cascade="all, delete-orphan"),
        uselist=False)
    marcrel: SynonymProperty[Optional[str]] = synonym('fk_roles')
    role: ColumnProperty = column_property(
        select(Role.role).where(Role.pk == fk_roles).scalar_subquery())
    #role = association_proxy('role_type', 'role')
    name: ColumnProperty = column_property(
        select(Author.name).where(Author.id == fk_authors).scalar_subquery())
